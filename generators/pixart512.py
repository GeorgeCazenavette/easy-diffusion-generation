...
class PixArt512(BaseGenerator):
    def __init__(self):
        super(PixArt512, self).__init__()

        self.pipe = diffusers.PixArtAlphaPipeline.from_pretrained(
            "PixArt-alpha/PixArt-XL-2-512x512", torch_dtype=torch.float16
        ).to("cuda")
    ...
    def generate(self, prompt, generator) -> Image:
        image = self.pipe(
            prompt=prompt, num_inference_steps=self.num_inference_steps
        ).images[0]
